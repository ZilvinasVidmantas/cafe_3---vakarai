/*
   ...
   veikimo principas:
     ...
     <from> nėra duotas, tuomet paieška pradedama nuo paskutinio elemento.
     Paieška vykdoma nuo galo.
 */

console.group('Veikimo pavyzdys');
{
  const temperatures = [5, 6, 2, -10, 1, -5, -6, -10, 4, 2, 5, 4];
  const indexOfMinus10 = temperatures.lastIndexOf(-10 /*, 4 */);
  if (indexOfMinus10 === -1) {
...
console.group('Praktinis pavyzdys. Nurodyto elemento pasikartojimų pašalinimas');
{
  ...
  const removeAll = function (arr, search) {
    let ii = arr.lastIndexOf(search);
    while (ii !== -1) {
      arr.splice(ii, 1);
      ii = arr.lastIndexOf(search, ii);
    }
  }
